...
func routeSearchImages(w http.ResponseWriter, r *http.Request, ver string, disabledEngines []engines.Name, secret string) error {
	// Capture start time.
	startTime := time.Now()
	...
	// Search for results.
	scrapedRes, err := search.Images(query, opts, catConf)
	if err != nil {
		// Server error.
		werr := writeResponseJSON(w, http.StatusInternalServerError, ErrorResponse{
