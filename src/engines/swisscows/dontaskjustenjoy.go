...
func performMagic(input string) (string, error) {
	vm := otto.New()
	if err := vm.Set("input_string", input); err != nil {
		return "", fmt.Errorf("performMagic(): failed setting otto vm: %w", err)
	}
	if _, err := vm.Run(`
	...
	`); err != nil {
		return "", fmt.Errorf("performMagic(): failed running otto vm: %w", err)
	}

	value, getErr := vm.Get("res")
	if getErr != nil {
		return "", fmt.Errorf("performMagic(): couldn't get result from javascript VM: %w", getErr)
	}

	value_str, toStrErr := value.ToString()
	if toStrErr != nil {
		return "", fmt.Errorf("performMagic(): couldn't convert returned value to string: %w", toStrErr)
	}
