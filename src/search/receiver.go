...
func createSuggestionsReceiver(rec *sync.Cond, engChan chan chan []result.SuggestionScraped, concMap *result.SuggestionConcurrentMap, enabledEnginesLen int) {
	for resChan := range engChan {
		go func() {
			for recArr := range resChan {
				if len(recArr) == 0 {
					log.Panic().
						Msg("Received an empty suggestion array")
					// ^PANIC - Assert because it should never happen.
				}
				...
				// Signal that the result was added.
				rec.L.Lock()
				rec.Signal()
				rec.L.Unlock()
			}
