...
func localeParamString(locale options.Locale) string {
	...
	for _, vl := range validLocales {
		if l == vl {
			return fmt.Sprintf("%v=%v", params.Locale, l)
		}
	}
	...
	return fmt.Sprintf("%v=%v", params.Locale, strings.ToLower(options.LocaleDefault.String()))
}
...
func safeSearchParamString(safesearch bool) string {
	if safesearch {
		...
		return fmt.Sprintf("%v=%v", params.SafeSearch, "2")
	}
}
